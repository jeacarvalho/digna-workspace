...
type SocialValuation struct {
	lifecycleManager lifecycle.LifecycleManager
}

func NewSocialValuation(lm lifecycle.LifecycleManager) *SocialValuation {
	return &SocialValuation{
		lifecycleManager: lm,
	}
}

func (sv *SocialValuation) RecordWork(entityID string, record *WorkRecord) error {
	if record.MemberID == "" {
	...
	db, err := sv.lifecycleManager.GetConnection(entityID)
	if err != nil {
...
func (sv *SocialValuation) GetTotalWorkByMember(entityID string, memberID string) (int64, int64, error) {
	if memberID == "" {
...
func (sv *SocialValuation) GetAllMembersWork(entityID string) (map[string]int64, error) {
	db, err := sv.lifecycleManager.GetConnection(entityID)
	if err != nil {
